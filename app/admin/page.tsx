...
export default function AdminDashboard() {
  ...
  const [membersCount, setMembersCount] = useState<number>(isDemo ? 12 : 0);
  const [newsletterStats, setNewsletterStats] = useState<any>(isDemo ? { uniqueSubscribers: 38, configured: true } : { uniqueSubscribers: 0 });
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      if (isDemo) {
        // En modo demo: cargamos datos del mock directamente, sin llamadas a API lentas
        ...
        // Simulamos miembros demo
        setMembers([
          { id: '1', nombre: 'María García', email: 'maria@ejemplo.com', status: 'activo', created_at: new Date(Date.now() - 86400000 * 2).toISOString() },
          { id: '2', nombre: 'Carlos López', email: 'carlos@ejemplo.com', status: 'activo', created_at: new Date(Date.now() - 86400000 * 5).toISOString() },
          { id: '3', nombre: 'Ana Martínez', email: 'ana@ejemplo.com', status: 'activo', created_at: new Date(Date.now() - 86400000 * 8).toISOString() },
          { id: '4', nombre: 'Pedro Sánchez', email: 'pedro@ejemplo.com', status: 'activo', created_at: new Date(Date.now() - 86400000 * 12).toISOString() },
          { id: '5', nombre: 'Laura Torres', email: 'laura@ejemplo.com', status: 'activo', created_at: new Date(Date.now() - 86400000 * 15).toISOString() },
        ]);
        setMembersCount(12);
        setNewsletterStats({ uniqueSubscribers: 38, configured: true });
      } else {
